...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onResume() {
        super.onResume()
        val sharedPreferences = this.getSharedPreferences("tes preference", MODE_PRIVATE)

        val loginTime = sharedPreferences.getLong("LOGIN_TIME", 0)
        val expiredTime = loginTime + sharedPreferences.getLong("EXPIRED_TIME", 0)
        ...
        if (currentTime > expiredTime) {
            ...
            Log.d(
                "LOGOUT",
                loginTime.toString() + " " + expiredTime.toString() + " " + currentTime.toString()
            )
